...
% Modified by Deepshihka Acharya, 2024

function SD = enPruneChannels(d,SD,fs,tInc,dRange,SNRthresh,SDrange,resetFlag)
...
for ii=1:nLambda
    ...
    for jj=1:length(lst1)
        ...
        if rhoSD(jj)<10
            [pow_avg,f]=plotFFT_getoutput(d(:,jj),fs);            
            [~,I]=max(pow_avg(f<2 & f>0.5)); freq=f(f<2 & f>0.5);
            if freq(I)==0.5 || freq(I)==2
                lst3(count)=jj;
                count=count+1;
            end
        end
